...
class ConcurrentWatchlistProcessorTest {
    ...
    private fun concurrentAddStock(threads: Int, retries: Int): Pair<DelegatingEventStore, List<Future<*>>> {
        ...
        val processors = (1..threads).map {
            service.submit {
                WatchlistProcessor(store, watchlist, retries).process(AddStock("$it"))
            }
        }
